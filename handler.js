...
if (data.exists)
m.reply(`${mssg.pluErrorA}\n\n${mssg.pluErrorC}\n*_${name}_*\n\n${mssg.pluErrorB}\n*_${m.sender}_*\n\n${mssg.pluErrorD}\n\`\`\`${format(e)}\`\`\`\n\n\n${mssg.botInfo}`.trim(), data.jid)
}}}
if (!opts['restrict'])
...
this.sendMessage(m.chat, {text: mssg.coinNoMoney,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: mssg.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
continue     
}
			
m.exp += xp
if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
this.sendMessage(m.chat, {text: `${mssg.coinNoLimit}buy`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: mssg.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
continue //Sin límite
}
if (plugin.level > _user.level) {
this.sendMessage(m.chat, {text: `Se requiere el nivel: *${plugin.level}*\nTu nivel actual es: *${_user.level}*\n\nPuedes subir de nivel usando el comando: *${usedPrefix}nivel*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: mssg.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
...
if (m.limit)
m.reply(`${mssg.coinGastar} ${m.limit} ${mssg.coinLimit}`)
}
if (m.money)
m.reply(`${mssg.coinGastar} ${m.money} ${mssg.coinMoney}`)  
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || mssg.aDescG) :
...
this.sendMessage(id, { text:`*${mssg.NumeroRaro}*`, mentions: [user] }, { quoted: null });          
...
"thumbnail": apii.data, 
"title": [mssg.botNombre, mssg.botDescrip, mssg.botInfo].getRandom(),
...
if (nk.isGroup == false) {
if (nk.status == "offer") {
let callmsg = await this.reply(nk.from, `${mssg.Llamada} *@${nk.from.split('@')[0]}*, ${nk.isVideo ? mssg.LlamadaB : mssg.LlamadaC} ${mssg.LlamadaD}`, false, { mentions: [nk.from] })
...
const antideleteMessage = `${mssg.rsDelete}
${mssg.antiDelete
...
let msg = {
rowner: mssg.AdvertenciaA,
owner: mssg.AdvertenciaA,
mods: mssg.AdvertenciaB,
premium: mssg.AdvertenciaC,
group: mssg.AdvertenciaD,
private: mssg.AdvertenciaE,
admin: mssg.AdvertenciaF,
botAdmin: mssg.AdvertenciaG,
unreg: mssg.AdvertenciaH,
restrict: mssg.Restrict,
}[type]
//if (msg) return m.reply(msg)
let tg = { quoted: m, userJid: conn.user.jid }
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: botInfo, body: [mssg.botAdvA, mssg.botAdvB, mssg.botAdvC].getRandom(), thumbnail: imgAdvs, sourceUrl: mycanal }}}}, tg)
