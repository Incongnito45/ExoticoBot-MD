...
if (!isNumber(user.joincount)) user.joincount = 1500
if (!isNumber(user.money)) user.money = 1500
if (!isNumber(user.limit)) user.limit = 1500 	       
...
if (!isNumber(user.glimit)) user.glimit = 1500
...
if (data.exists)
m.reply(`${IDexotic.pluErrorA}\n\n${IDexotic.pluErrorC}\n*_${name}_*\n\n${IDexotic.pluErrorB}\n*_${m.sender}_*\n\n${IDexotic.pluErrorD}\n\`\`\`${format(e)}\`\`\`\n\n\n${IDexotic.botInfo}`.trim(), data.jid)
}}}
if (!opts['restrict'])
...
m.reply(`ESTAS BANEADO(A), NO USARAS EL BOT POR UN TIEMPO.`)
user.antispam++	
return
}
...
this.sendMessage(m.chat, {text: IDexotic.coinNoMoney,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: IDexotic.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
continue     
}
			
m.exp += xp
if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
this.sendMessage(m.chat, {text: `${IDexotic.coinNoLimit(usedPrefix)}buy`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: IDexotic.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
continue //Sin límite
}
if (plugin.level > _user.level) {
this.sendMessage(m.chat, {text: `Se requiere el nivel: *${plugin.level}*\nTu nivel actual es: *${_user.level}*\n\nPuedes subir de nivel usando el comando: *${usedPrefix}nivel*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: IDexotic.botDescrip, previewType: 0, thumbnail: menusRB, sourceUrl: exoticoAll }}}, { quoted: m })         
...
if (m.limit)
m.reply(`${IDexotic.coinGastar} ${m.limit} ${IDexotic.coinLimit}`)
}
if (m.money)
m.reply(`${IDexotic.coinGastar} ${m.money} ${IDexotic.coinMoney}`)  
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || IDexotic.aDescG) :
...
this.sendMessage(id, { text:`*${IDexotic.NumeroRaro}*`, mentions: [user] }, { quoted: null });          
...
"thumbnail": apii.data, 
"title": [IDexotic.botNombre, IDexotic.botDescrip, IDexotic.botInfo].getRandom(),
...
if (nk.isGroup == false) {
if (nk.status == "offer") {
let callmsg = await this.reply(nk.from, `${IDexotic.Llamada} *@${nk.from.split('@')[0]}*, ${nk.isVideo ? IDexotic.LlamadaB : IDexotic.LlamadaC} ${IDexotic.LlamadaD}`, false, { mentions: [nk.from] })
...
const antideleteMessage = `${IDexotic.rsDelete}
${IDexotic.antiDelete

${IDexotic.Usuario}
• @${participant.split`@`[0]}

${IDexotic.antiDeleteA}
${IDexotic.rsDelete}`.trim();
...
let msg = {
rowner: IDexotic.AdvertenciaA,
owner: IDexotic.AdvertenciaA,
mods: IDexotic.AdvertenciaB,
premium: IDexotic.AdvertenciaC,
group: IDexotic.AdvertenciaD,
private: IDexotic.AdvertenciaE,
admin: IDexotic.AdvertenciaF,
botAdmin: IDexotic.AdvertenciaG,
unreg: IDexotic.AdvertenciaH,
restrict: IDexotic.Restrict,
}[type]
//if (msg) return m.reply(msg)
let tg = { quoted: m, userJid: conn.user.jid }
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: botInfo, body: [IDexotic.botAdvA, IDexotic.botAdvB, IDexotic.botAdvC].getRandom(), thumbnail: imgAdvs, sourceUrl: mycanal }}}}, tg)
