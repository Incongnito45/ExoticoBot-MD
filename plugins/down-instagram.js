...
let tXXxt = `${IDexotic.aquiVideo}`.trim()  
...
const tXXxt = `${IDexotic.aquiVideo}`.trim();
...
const txt2 = `${IDexotic.aquiVideo}`.trim();
...
const txt4 = `${IDexotic.aquiVideo}`.trim();
...
const txt1 = `${IDexotic.aquiVideo}`.trim();
await conn.sendFile(m.chat, videoig, 'error.mp4', txt1, m);
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
} catch (e) {
conn.sendMessage(m.chat, {text: IDexotic.ErrorA, edit: key});
