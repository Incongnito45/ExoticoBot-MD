...
number = user.split('@')[0]
if(user === conn.user.jid) return conn.reply(m.chat, IDexotic.banUserE, null, { mentions: [user] })   
...
await conn.reply(m.chat, IDexotic.banUserA, null, { mentions: [user] })   
//await conn.reply(user, lenguajeGB.smsPropban6(number, usr), null, { mentions: [user, m.sender] })
}} catch (e) {
await conn.reply(m.chat, IDexotic.respoUser2, null, m)
console.log(e) 
}}
