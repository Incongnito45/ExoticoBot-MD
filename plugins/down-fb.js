...
if (!args[0]) return conn.reply(m.chat, `᥀·࣭࣪̇˖☁️◗ 𝙄𝙣𝙜𝙧𝙚𝙨𝙚 𝙪𝙣 𝙚𝙣𝙡𝙖𝙘𝙚 𝙫𝙖𝙡𝙞𝙙𝙤 𝙙𝙚 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠 𝙥𝙖𝙧𝙖 𝙙𝙚𝙨𝙘𝙖𝙧𝙜𝙖𝙧.\n• 𝙋𝙤𝙧 𝙚𝙟𝙚𝙢𝙥𝙡𝙤: *${usedPrefix + command} https://www.facebook.com/watch?v=636541475139*`, fkontak, m)
if (!args[0].match(/www.facebook.com|fb.watch/g)) return conn.reply(m.chat, `᥀·࣭࣪̇˖☁️◗ 𝙀𝙡 𝙚𝙣𝙡𝙖𝙘𝙚 𝙞𝙣𝙜𝙧𝙚𝙨𝙖𝙙𝙤 𝙣𝙤 𝙣𝙤 𝙚𝙨 𝙫𝙖𝙡𝙞𝙙𝙤.\n• 𝙋𝙤𝙧 𝙚𝙟𝙚𝙢𝙥𝙡𝙤: *${usedPrefix + command} https://www.facebook.com/watch?v=636541475139*`, fkontak, m)
try { 
...
conn.sendFile(m.chat, r2es, 'error.mp4', `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
handler.limit = 3
} catch (err3) {
try {
...
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
handler.limit = 3
}} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
handler.limit = 3
} catch (err5) {
try {
const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, `᥀·࣭࣪̇˖☁️◗ 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠... ✨`, m);
handler.limit = 3
} catch (err6) {
await conn.reply(m.chat, `᥀·࣭࣪̇˖⚠️◗ 𝙀𝙡 𝙘𝙤𝙢𝙖𝙣𝙙𝙤 𝙝𝙖 𝙛𝙖𝙡𝙡𝙖𝙙𝙤 𝙘𝙤𝙢𝙥𝙡𝙚𝙩𝙖𝙢𝙚𝙣𝙩𝙚, 𝙧𝙚𝙥𝙤𝙧𝙩𝙖𝙡𝙤 𝙖𝙡 𝙘𝙧𝙚𝙖𝙙𝙤𝙧.`, fkontak, m)
