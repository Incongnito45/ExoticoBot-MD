...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
 if (!text) return conn.reply(m.chat, `${IDexotic.faltaTexto}\n\n*${usedPrefix + command} Erika lundmoen - yad*`,  m, fkontak, )
  try {
    ...
    m.react('✅') 
    const texto1 = `${additionalText}`.trim();
...
handler.command = /^(playvideo|playaudio)$/i;
