...
let exotico = Math.floor(Math.random() * 3000)
global.db.data.users[m.sender].exp += exotico * 1  
let time = global.db.data.users[m.sender].lastwork + 600000
if (new Date - global.db.data.users[m.sender].lastwork < 600000) throw `᥀·࣭࣪̇˖⏱️◗ 𝙑𝙪𝙚𝙡𝙫𝙖 𝙚𝙣: ${msToTime(time - new Date())} 𝙥𝙖𝙧𝙖 𝙫𝙤𝙡𝙫𝙚𝙧 𝙖 𝙩𝙧𝙖𝙗𝙖𝙟𝙖𝙧.`

await conn.reply(m.chat, `*${pickRandom(global.work)}* ${exotico} XP`, fkontak, pp, m)
