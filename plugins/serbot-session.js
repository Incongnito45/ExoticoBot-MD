...
try {
  await fs.rmdir("./MultiBotOFC/" + uniqid, { recursive: true, force: true })
    await conn.sendMessage(m.chat, { text: IDexotic.sesionClear }, { quoted: m })
      await conn.sendMessage(m.chat, { text : IDexotic.serbotClose } , { quoted: m })
} catch(err) {
if (err.code === 'ENOENT' && err.path === `./MultiBotOFC/${uniqid}`) {
await conn.sendMessage(m.chat, { text: "Usted no es Sub Bot" }, { quoted: m })
} else {
console.error(userS + ' ' + IDexotic.rsError, err)
}
}
}
}
