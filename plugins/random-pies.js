...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw `Los comandos para adultos estan desactivdos...`
let url = pies[Math.floor(Math.random() * pies.length)]
conn.sendButton(m.chat, command, wm, url, [['🦶🏻 SIGUIENTE 🦶🏻', `/${command}`]], null, null, m)
}
handler.help = ['pies']
