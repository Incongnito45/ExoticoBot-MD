...
if (!args[0]) return conn.reply(m.chat, `${IDexotic.faltaLink} Facebook\n${IDexotic.porEjemplo}\n${usedPrefix + command} https://www.facebook.com/watch?v=636541475139`, fkontak, m)
if (!args[0].match(/www.facebook.com|fb.watch/g)) return conn.reply(m.chat, `${IDexotic.faltaLink} Facebook\n${IDexotic.porEjemplo}\n*${usedPrefix + command} https://www.facebook.com/watch?v=636541475139*`, fkontak, m)
try { 
...
conn.sendFile(m.chat, r2es, 'error.mp4', `${IDexotic.aquiVideo}`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `${IDexotic.aquiVideo}`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `${IDexotic.aquiVideo}`, m);
handler.limit = 3
} catch (err3) {
try {
...
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `${IDexotic.aquiVideo}`, m);
handler.limit = 3
}} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `${IDexotic.aquiVideo}`, m);
handler.limit = 3
} catch (err5) {
try {
const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, `${IDexotic.aquiVideo}`, m);
handler.limit = 3
} catch (err6) {
