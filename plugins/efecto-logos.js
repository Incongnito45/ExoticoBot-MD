...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  ...
  if (!effect) throw '᥀·࣭࣪̇˖🗿◗ 𝙄𝙣𝙜𝙧𝙚𝙨𝙚 𝙚𝙡 𝙘𝙤𝙢𝙖𝙣𝙙𝙤: #menulogos'
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `᥀·࣭࣪̇˖🪐◗ 𝙎𝙚𝙡𝙚𝙘𝙘𝙞𝙤𝙣𝙚 𝙚𝙡 𝙚𝙛𝙚𝙘𝙩𝙤.\n\n• 𝙋𝙤𝙧 𝙚𝙟𝙚𝙢𝙥𝙡𝙤:\n${usedPrefix + command} ${effect} ExoticoBot`;  
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw `᥀·࣭࣪̇˖🛸◗ 𝘾𝙧𝙚𝙖𝙣𝙙𝙤, 𝙚𝙨𝙥𝙚𝙧𝙚 𝙪𝙣 𝙢𝙤𝙢𝙚𝙣𝙩𝙤...` })
   if (typeof res == 'number') throw res == -1 ? `᥀·࣭࣪̇˖🪐◗ 𝙎𝙚𝙡𝙚𝙘𝙘𝙞𝙤𝙣𝙚 𝙚𝙡 𝙚𝙛𝙚𝙘𝙩𝙤.\n\n• 𝙋𝙤𝙧 𝙚𝙟𝙚𝙢𝙥𝙡𝙤:\n${usedPrefix + command} ${effect} ExoticoBot` : `᥀·࣭࣪̇˖🪐◗ 𝙋𝙖𝙧𝙖 𝙪𝙨𝙖𝙧 𝙚𝙨𝙩𝙚 𝙡𝙤𝙜𝙤, 𝙪𝙨𝙚𝙡𝙤 𝙙𝙚 𝙡𝙖 𝙨𝙞𝙜𝙪𝙞𝙚𝙣𝙩𝙚 𝙢𝙖𝙣𝙚𝙧𝙖.\n\n• 𝙋𝙤𝙧 𝙚𝙟𝙚𝙢𝙥𝙡𝙤:\n${usedPrefix + command} ${effect} Exotico|Bot*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `᥀·࣭࣪̇˖🌇◗ 𝙀𝙭𝙤𝙩𝙞𝙘𝙤𝘽𝙤𝙩-𝙈𝘿... ✨`}, {quoted: m});  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
