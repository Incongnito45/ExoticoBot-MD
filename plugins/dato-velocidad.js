...
try {
let enlace = { contextInfo: { externalAdReply: {title: IDexotic.botNombre, body: IDexotic.botDescrip, sourceUrl: exoticoAll, thumbnailUrl: await(await fetch(imagenRB)).buffer() }}}
let enlace2 = { contextInfo: { externalAdReply: { showAdAttribution: true, mediaUrl: paypal, mediaType: 'VIDEO', description: '', title: IDexotic.botNombre, body: IDexotic.botDescrip, thumbnailUrl: await(await fetch(imagenRB)).buffer(), sourceUrl: exoticoAll }}}
...
await conn.sendFile(m.chat, gataImg, 'pp.jpg', caption, fkontak, false, { contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: IDexotic.botInfo, body: IDexotic.botDescrip, previewType: 0, thumbnail: imagenRB, sourceUrl: exoticoAll }}})
} catch (e) {
await conn.reply(m.chat, `*ALGO SALIÓ MAL. ERRORS FOUND.*\n\n\`\`\`REPORTE ESTE COMANDO ${usedPrefix + command} CON EL COMANDO ${usedPrefix}reporte\`\`\``, m)
console.log(e)
}}
