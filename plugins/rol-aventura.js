...
if (user.health < 80) return await conn.reply(m.chat, `TU SALUD ESTA POR DEBAJO DE: *80!!*\nUSA EL COMANDO: *${usedPrefix}heal* PARA CURARTE...`, fkontak, imgr, m)    
if (new Date - user.lastadventure <= cooldown) return await conn.reply(m.chat, `${IDexotic.rpgTiempo}${IDexotic.Tiempo} ${timers.toTimeString()}`, fkontak, imgr, m)    
    const rewards = reward(user)
    let text = `ESTAS VIAJANDO EN: ${kt[1][0].name}*
